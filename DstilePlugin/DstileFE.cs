...
namespace DstileGUI
{
    ...
    class DstileFrontEnd : Form
    {
        ...
        private RadioButton demRadioButton;
        ...
        public DstileFrontEnd(DstilePlugin source)
        {
            ...
            this.tileListBox.Items.AddRange(this.lztsdvalues);
            this.formatListBox.Items.AddRange(this.formatvalues);
            ...
            this.srcplugin = source;
            this.LoadSaveXmlConfig(true);
        }
        ...
        private void LoadSaveXmlConfig(bool load)
        {
            try
            {
                ...
                //					{
                string PluginDirectory = this.srcplugin.PluginDirectory;
                ...
                if (!File.Exists(settings))
                {
                    ...
                    fwtools.InnerText = this.fwlocText.Text;
                ...
                if (File.Exists(settings))
                {
                    ...
                    {
                        if (load)
                            this.fwlocText.Text = node.InnerText;
                            
                        else
                            node.InnerText = this.fwlocText.Text;
                    ...
                    else
                    {
                        ...
                        XmlNode layersNode = this.srcplugin.DstileLayers.ToXml(doc);
        ...
        private void loadButton_Click(object sender, EventArgs e)
        {
            ...
            if (this.openFileDialog1.ShowDialog(this) == DialogResult.OK)
            {
                ...
                this.locateData(fileName);
            }
        }
        
        public void locateData(string fileName)
        {
            this.inputFileTextBox.Text = fileName;
            if (File.Exists(fileName)) this.currentStatusLabel.Text = "Data located";
            ...
            process.Dispose();
            */
            if (Environment.GetEnvironmentVariable("GDAL_DATA") == null)
                Environment.SetEnvironmentVariable("GDAL_DATA", Path.Combine(this.fwlocText.Text, "data\\"));
            gdal.AllRegister();

            Console.WriteLine("GDAL Drivers:"+gdal.GetDriverCount());
            Dataset dataset = gdal.Open(strfileName, 0);
            ...
            try
            {
                ...
                this.northlab.Text = Convert.ToString(Math.Round(topleft[1], 2));
                this.westlab.Text = Convert.ToString(Math.Round(topleft[0], 2));
                this.southlab.Text = Convert.ToString(Math.Round(bottomright[1], 2));
                this.eastlab.Text = Convert.ToString(Math.Round(bottomright[0], 2));

                //TODO: Make this an educated guess based on image extent
                //and resolution
                double width = bottomright[0] - topleft[0];
                for (int i = 0; i < this.lztsdvalues.Length; i++)
                    if (width > Convert.ToDouble(this.lztsdvalues[i].ToString()))
                    {
                        this.tileListBox.SelectedIndex = i;
                        break;
                    }
                if (this.tileListBox.SelectedItem == null) this.tileListBox.SelectedIndex = this.lztsdvalues.Length - 1;
            }
            catch (Exception ex)
            {
        ...
        private void processButton_Click(object sender, EventArgs e)
        {
            ...
            */
            ...
            if (this.imageRadioButton.Checked)
                this.loadGDALVrt();
            else if (this.demRadioButton.Checked) this.loadGDALVrtDem();
        }
        ...
        private void chooseDestButton_Click(object sender, EventArgs e)
        {
            ...
            if (folderDialog.ShowDialog(this) == DialogResult.OK)
            {
                ...
                this.destFolderTextBox.Text = dirName;
                if (this.wwIsValidated(dirName))
                    this.currentStatusLabel.Text = "NEXTIMAGE Installation located";
                else
                    this.currentStatusLabel.Text = "NEXTIMAGE Installation not found";
        ...
        {
            String dirname = this.destFolderTextBox.Text;
            if (this.wwIsValidated(dirname)) this.updateXMLPath();
        }

        private void inputFileTextBox_TextChanged(object sender, EventArgs e)
        {
            String fname = this.inputFileTextBox.Text;
            if (File.Exists(fname)) this.currentStatusLabel.Text = "Data located";
        }
        ...
        private void updateXMLPath()
        {
            if (!this.imageRadioButton.Checked)
            {
                this.targetXMLTextBox.Text = Path.Combine(Path.GetDirectoryName(Application.ExecutablePath), "Config\\Earth.xml");
                this.destFolderTextBox.Text = Path.Combine(Path.GetDirectoryName(Application.ExecutablePath), "Cache\\Earth\\SRTM\\" + this.layerNameTextBox.Text);
            }
            else
            {
                this.targetXMLTextBox.Text = Path.Combine(Path.GetDirectoryName(Application.ExecutablePath), "Config\\Earth\\" + this.layerNameTextBox.Text + ".xml");
                this.destFolderTextBox.Text = Path.Combine(Path.GetDirectoryName(Application.ExecutablePath), "Data\\Earth\\" + this.layerNameTextBox.Text);
            }
        }
        ...
        private void openNotepad()
        {
            ...
            psi.Arguments = this.targetXMLTextBox.Text;
        ...
        private void dstileProcess()
        {
            ...
            this.timer1.Start();
            ProcessStartInfo psi = new ProcessStartInfo(Path.Combine(this.fwlocText.Text,"bin//dstile.exe"));
            psi.UseShellExecute = false;
            psi.CreateNoWindow = true;
            Process process = new Process();
            ...
            psi.RedirectStandardOutput = true;
            if (!psi.EnvironmentVariables.ContainsKey("GDAL_DATA"))
                psi.EnvironmentVariables.Add("GDAL_DATA", Path.Combine(this.fwlocText.Text, "data"));
            if (!psi.EnvironmentVariables.ContainsKey("PROJ_LIB"))
                psi.EnvironmentVariables.Add("PROJ_LIB", Path.Combine(this.fwlocText.Text, "proj_lib"));
            if (!psi.EnvironmentVariables.ContainsKey("GEOTIFF_CSV"))
                psi.EnvironmentVariables.Add("GEOTIFF_CSV", Path.Combine(this.fwlocText.Text, "data"));
            if (!psi.EnvironmentVariables.ContainsKey("GDAL_DRIVER_PATH"))
                psi.EnvironmentVariables.Add("GDAL_DRIVER_PATH", Path.Combine(this.fwlocText.Text, "gdal_plugins"));
            process.EnableRaisingEvents = true;
            ...
            if (process.ExitCode == 0)
            {
                ...
                RenderableObjectList layers = ConfigurationLoader.
                    getRenderableFromLayerFile(this.targetXMLTextBox.Text, this.srcplugin.ParentApplication.WorldWindow.CurrentWorld, this.srcplugin.ParentApplication.WorldWindow.Cache);
                this.srcplugin.ParentApplication.WorldWindow.CurrentWorld.RenderableObjects.Add(layers);
                //this.processButton.Text = "Process";
            }
        ...
        private void loadGDALVrt()
        {
            ...
            gdal.AllRegister();
            Dataset src_ds = gdal.Open(this.inputFileTextBox.Text,gdalconst.GA_ReadOnly);
            ...
            if (this.srcProjNeeded)
                src_srs = new SpatialReference(this.srcProjTextBox.Text);
            else
                src_srs = new SpatialReference(src_ds.GetProjection());

            string epsgwkt = "GEOGCS[\"WGS 84\",DATUM[\"WGS_1984\",SPHEROID[\"WGS 84\",6378137,298.2572235629972,AUTHORITY[\"EPSG\",\"7030\"]],AUTHORITY[\"EPSG\",\"6326\"]],PRIMEM[\"Greenwich\",0],UNIT[\"degree\",0.0174532925199433],AUTHORITY[\"EPSG\",\"4326\"]]";
            SpatialReference dst_srs = new SpatialReference(epsgwkt);
            Dataset dst_ds = gdal.AutoCreateWarpedVRT(src_ds, src_ds.GetProjection(),
            ...
            double lztsd = Convert.ToDouble(this.tileListBox.SelectedItem);
            string format = Convert.ToString(this.formatListBox.SelectedItem);
            int tilelevels = (int)Math.Ceiling(Math.Log(lztsd / baseres) / Math.Log(2.0));
            World curworld = this.srcplugin.ParentApplication.WorldWindow.CurrentWorld;
            GDALImageStore gdalstore = 
                new GDALImageStore(this.layerNameTextBox.Text, this.inputFileTextBox.Text,
                dst_ds,format);
            gdalstore.LevelCount = tilelevels;
            gdalstore.LevelZeroTileSizeDegrees = lztsd;
            gdalstore.DataDirectory = Path.Combine(this.srcplugin.PluginDirectory,
                String.Format("Data\\{0}\\{1}\\",
                curworld.Name, this.layerNameTextBox.Text));
            ...
            QuadTileSet qts = new QuadTileSet(this.layerNameTextBox.Text,
                curworld,0,
                Convert.ToDouble(this.northlab.Text),
                Convert.ToDouble(this.southlab.Text),
                Convert.ToDouble(this.westlab.Text),
                Convert.ToDouble(this.eastlab.Text),true,
                stores);
            qts.ColorKey = 0;
            qts.ColorKeyMax = 10;
        ...
        private void loadGDALVrtDem()
        {
            ...
            World curworld = this.srcplugin.ParentApplication.WorldWindow.CurrentWorld;
            string datadir = Path.Combine(this.srcplugin.PluginDirectory,
                String.Format("Data\\{0}\\DEM\\{1}\\",
                curworld.Name, this.layerNameTextBox.Text));

            GDALTerrainAccessor gdalterrain = new GDALTerrainAccessor(Convert.ToDouble(this.northlab.Text),
                Convert.ToDouble(this.southlab.Text),
                Convert.ToDouble(this.westlab.Text),
                Convert.ToDouble(this.eastlab.Text), dst_ds, this.layerNameTextBox.Text, lztsd, tilelevels, datadir);
        ...
        private void getFWLoc()
        {
            ...
            if (folderDialog.ShowDialog(this) == DialogResult.OK)
            {
                ...
                {
                    this.fwlocText.Text = dirName;
                    this.LoadSaveXmlConfig(false);
                }
                else
