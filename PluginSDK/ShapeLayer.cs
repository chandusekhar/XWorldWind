...
namespace WorldWind
{
	...
	public class ShapeLayer : RenderableObject
	{
		...
		Hashtable _currentPlacenames = new Hashtable();
		Hashtable _currentPolygons = new Hashtable();
		Hashtable _currentBoundaries = new Hashtable();
		Font drawingFont;
		...
		/// <summary>
		/// Initializes a new instance of the <see cref= "T:WorldWind.ShapeLayer"/> class.
		/// </summary>
	...
	class ShapeRecord
	{
		...
		Hashtable _scalarData = new Hashtable();
		#endregion
	...
	class ShapeIndex
	{
		...
		public void Save(string indexFilePath)
		{
			...
			using( BinaryWriter writer = new BinaryWriter(stream, System.Text.Encoding.ASCII) ) 
			{
				...
				{
					writer.Write((string) curRecord.ID);
					if(curRecord.Name == null)
						writer.Write(String.Empty);
					else
						writer.Write((string) curRecord.Name);

					writer.Write((double) curRecord.West);
					writer.Write((double) curRecord.South);
					writer.Write((double) curRecord.East);
					writer.Write((double) curRecord.North);

					if(curRecord.PolygonFile == null)
						writer.Write(String.Empty);
					else
						writer.Write((string) curRecord.PolygonFile);

					if(curRecord.BoundaryFile == null)
						writer.Write(String.Empty);
					else
						writer.Write((string) curRecord.BoundaryFile);

					writer.Write((int) curRecord.MetaData.Count);
					...
					writer.Write((int) curRecord.ScalarData.Count);
					foreach(string key in curRecord.ScalarData.Keys) 
					{
						writer.Write(key);
