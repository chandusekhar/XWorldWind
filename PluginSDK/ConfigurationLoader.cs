...
namespace WorldWind
{
	...
	public class ConfigurationLoader
	{
            ...
            try
            {
                ...
                if (worldIter.Count > 0)
                {
                    ...
                    TerrainAccessor[] terrainAccessor = getTerrainAccessorsFromXPathNodeIterator(worldIter.Current.Select("TerrainAccessor"),
                        Path.Combine(cache.CacheDirectory, worldName));

                    World newWorld = new World(
                        worldName,
                        new SharpDX.Vector3(0, 0, 0),
                        new SharpDX.Quaternion(0, 0, 0, 0),
                        equatorialRadius,
		...
		private static bool ParseBool(string booleanString)
		{
            ...
            booleanString = booleanString.Trim().ToLower(CultureInfo.InvariantCulture);
                ...
                */
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        {
                            Net.WebDownload download = new Net.WebDownload(layerFile);
                ...
                if (iter.Count > 0)
                {
                    ...
                    if (redirect != null && redirect.Length > 0)
                    {
                        ...
                        try
                        {
                            ...
                            Net.WebDownload download = new Net.WebDownload(redirect);
                            ...
                            download.DownloadFile(tempDownloadFile.FullName, Net.DownloadType.Unspecified);
                    ...
                    else
                    {
                        ...
                        {
                            if (String.Compare(renderPriorityString, "Icons", false, CultureInfo.InvariantCulture) == 0)
                            {
                                parentRenderable.RenderPriority = RenderPriority.Icons;
                            }
                            else if (String.Compare(renderPriorityString, "LinePaths", false, CultureInfo.InvariantCulture) == 0)
                            {
                                parentRenderable.RenderPriority = RenderPriority.LinePaths;
                            }
                            else if (String.Compare(renderPriorityString, "Placenames", false, CultureInfo.InvariantCulture) == 0)
                            {
                                parentRenderable.RenderPriority = RenderPriority.Placenames;
                            }
                            else if (String.Compare(renderPriorityString, "AtmosphericImages", false, CultureInfo.InvariantCulture) == 0)
                            {
                                parentRenderable.RenderPriority = RenderPriority.AtmosphericImages;
                            }
                            else if (String.Compare(renderPriorityString, "TerrainMappedImages", false, CultureInfo.InvariantCulture) == 0)
                            {
                                //Added by James Evans for 84 RADES
                                parentRenderable.RenderPriority = RenderPriority.TerrainMappedImages;
                            }
                        }
                        ...
                        addCircleFeature(iter.Current.Select("CircleFeature"), parentWorld, parentRenderable);
		...
		private static RenderableObjectList addChildLayerSet(XPathNodeIterator iter, World parentWorld, RenderableObjectList parentRenderable, Cache cache)
		{
			if(iter.Count > 0)
			{
				while(iter.MoveNext())
				{
                    ...
                    addCircleFeature(iter.Current.Select("CircleFeature"), parentWorld, rol);
        ...
        private static ImageStore getImageStoreFromXPathNodeIterator(string name, XPathNodeIterator imageAccessorIter, RenderableObjectList parentRenderable, Cache cache)
        {
			...
			SharpDX.Direct3D9.Format format = World.Settings.TextureFormat;
			if (textureFormatSpec != null)
			{
				switch(textureFormatSpec)
				{
					case "A16B16G16R16":
						format = SharpDX.Direct3D9.Format.A16B16G16R16;
						break;
					case "A16B16G16R16F":
						format = SharpDX.Direct3D9.Format.A16B16G16R16F;
						break;
                    case "A8B8G8R8":
                        format = SharpDX.Direct3D9.Format.A8B8G8R8;
                        break;
                    case "R16F":
                        format = SharpDX.Direct3D9.Format.R16F;
                        break;
                    case "R8G8B8":
                        format = SharpDX.Direct3D9.Format.R8G8B8;
                        break;
                    case "A8R8G8B8":
                        format = SharpDX.Direct3D9.Format.A8R8G8B8;
                        break;
                    default:
						Log.Write(Log.Levels.Warning, "Unsupported texture format '" + textureFormatSpec + "', reverting to WW default ("+format+")");
			...
			if(wmsAccessorIter.Count > 0)
			{
                ...
                wmsLayerStore.Password = password;
		...
		private static void addQuadTileLayersFromXPathNodeIterator(XPathNodeIterator iter, World parentWorld, RenderableObjectList parentRenderable, Cache cache)
		{
			while(iter.MoveNext())
			{
                ...
                //TODO: Add additional parameters to the effect
		...
		private static void addImageLayersFromXPathNodeIterator(XPathNodeIterator iter, World parentWorld, RenderableObjectList parentRenderable)
		{
			if(iter.Count > 0)
			{
				while(iter.MoveNext())
				{
                    ...
                    //Added by James Evans for 84 RADES
                    string renderPriorityString = getInnerTextFromFirstChild(iter.Current.Select("RenderPriority"));
					...
					if(boundingBoxIter.Count > 0)
					{
                        ...
                        if (renderPriorityString != null)
                        {
                            if (String.Compare(renderPriorityString, "Icons", false, CultureInfo.InvariantCulture) == 0)
                            {
                                im.RenderPriority = RenderPriority.Icons;
                            }
                            else if (String.Compare(renderPriorityString, "LinePaths", false, CultureInfo.InvariantCulture) == 0)
                            {
                                im.RenderPriority = RenderPriority.LinePaths;
                            }
                            else if (String.Compare(renderPriorityString, "Placenames", false, CultureInfo.InvariantCulture) == 0)
                            {
                                im.RenderPriority = RenderPriority.Placenames;
                            }
                            else if (String.Compare(renderPriorityString, "AtmosphericImages", false, CultureInfo.InvariantCulture) == 0)
                            {
                                im.RenderPriority = RenderPriority.AtmosphericImages;
                            }
                            else if (String.Compare(renderPriorityString, "TerrainMappedImages", false, CultureInfo.InvariantCulture) == 0)
                            {
                                im.RenderPriority = RenderPriority.TerrainMappedImages;
                            }
                        }
			...
			if(iter.Count > 0)
			{
				while(iter.MoveNext())
				{
					...
					if(startXString != null && startYString != null)
					{
						...
						{
                            if (alignmentString.ToLower(CultureInfo.InvariantCulture).Equals("left"))
							{
								overlay.Alignment = ScreenAlignment.Left;
							}
                            else if (alignmentString.ToLower(CultureInfo.InvariantCulture).Equals("right"))
							{
		...
		*/
		...
		private static SharpDX.Direct3D9.FontDescription getDisplayFont(XPathNodeIterator iter)
		{
			SharpDX.Direct3D9.FontDescription fd = new SharpDX.Direct3D9.FontDescription();
			...
			if(iter.MoveNext())
			{
				...
				if(styleIter.Count > 0)
				{
					...
					if(isBoldString != null)
					{
						...
						if(isBold)
							fd.Weight = SharpDX.Direct3D9.FontWeight.Bold;
					}
				...
				{
					fd.Weight = SharpDX.Direct3D9.FontWeight.Regular;
				}
			}
		...
		private static void addTiledPlacenameSet(XPathNodeIterator iter, World parentWorld, RenderableObjectList parentRenderable)
		{
			if(iter.Count > 0)
			{
				while(iter.MoveNext())
				{
					...
					SharpDX.Direct3D9.FontDescription fd = getDisplayFont(iter.Current.Select("DisplayFont"));
		...
		private static void addIcon(XPathNodeIterator iter, World parentWorld, RenderableObjectList parentRenderable, Cache cache)
		{
			if(iter.Count > 0)
			{
				while(iter.MoveNext())
				{
                    ...
                    if (textureFilePath.Length > 0 && !Path.IsPathRooted(textureFilePath) && !textureFilePath.ToLower(CultureInfo.InvariantCulture).StartsWith("http://"))
		...
		private static void addLineFeature(XPathNodeIterator iter, World parentWorld, RenderableObjectList parentRenderable)
		{
			if(iter.Count > 0)
			{
				while(iter.MoveNext())
				{
                    ...
                    {
                        if (altitudemodeString.ToLower(CultureInfo.InvariantCulture).Equals("absolute")) lf.AltitudeMode = AltitudeMode.Absolute;
                        if (altitudemodeString.ToLower(CultureInfo.InvariantCulture).Equals("relativetoground")) lf.AltitudeMode = AltitudeMode.RelativeToGround;
                        if (altitudemodeString.ToLower(CultureInfo.InvariantCulture).Equals("clampedtoground")) lf.AltitudeMode = AltitudeMode.ClampedToGround;
                    }
		...
		private static void addPolygonFeature(XPathNodeIterator iter, World parentWorld, RenderableObjectList parentRenderable)
		{
			if(iter.Count > 0)
			{
				while(iter.MoveNext())
				{
                    ...
                    if (altitudeModeString != null && altitudeModeString.Length > 0)
                    {
                        if (altitudeModeString.ToLower(CultureInfo.InvariantCulture).Equals("absolute")) pf.AltitudeMode = AltitudeMode.Absolute;
                        if (altitudeModeString.ToLower(CultureInfo.InvariantCulture).Equals("relativetoground")) pf.AltitudeMode = AltitudeMode.RelativeToGround;
                        if (altitudeModeString.ToLower(CultureInfo.InvariantCulture).Equals("clampedtoground")) pf.AltitudeMode = AltitudeMode.ClampedToGround;
                    }
        ...
        private static void addCircleFeature(XPathNodeIterator iter, World parentWorld, RenderableObjectList parentRenderable)
        {
            if (iter.Count > 0)
            {
                while (iter.MoveNext())
                {
                    ...
                    double lat = Convert.ToDouble(getInnerTextFromFirstChild(iter.Current.Select("Latitude")));
                    double lon = Convert.ToDouble(getInnerTextFromFirstChild(iter.Current.Select("Longitude")));
                    double alt = Convert.ToDouble(getInnerTextFromFirstChild(iter.Current.Select("Altitude")));
                    double radius = Convert.ToDouble(getInnerTextFromFirstChild(iter.Current.Select("Radius")));
                    int numPoints = Convert.ToInt32(getInnerTextFromFirstChild(iter.Current.Select("NumPoints")));
                    string distanceAboveSurfaceString = getInnerTextFromFirstChild(iter.Current.Select("DistanceAboveSurface"));
                    string minimumDisplayAltitudeString = getInnerTextFromFirstChild(iter.Current.Select("MinimumDisplayAltitude"));
                    string maximumDisplayAltitudeString = getInnerTextFromFirstChild(iter.Current.Select("MaximumDisplayAltitude"));
                    string opacityString = getInnerTextFromFirstChild(iter.Current.Select("Opacity"));
                    ...
                    pf = new PolygonFeature(name, parentWorld, Angle.FromDegrees(lat), Angle.FromDegrees(lon), alt, radius, numPoints, c);
