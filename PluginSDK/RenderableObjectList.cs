...
namespace WorldWind
{
	...
	public class RenderableObjectList : RenderableObject
	{
		...
		{
			get{ return this.m_disableExpansion; }
			set{ this.m_disableExpansion = value; }
		}
		...
		/// <summary>
		/// Initializes a new instance of the <see cref= "T:WorldWind.RenderableObjectList"/> class.
		/// </summary>
			...
			) : base(name, new Vector3(0,0,0), new Quaternion())
		{
            ...
            this.m_RefreshTimer = new System.Timers.Timer(
        ...
        {
            menu.MenuItems.Add("Check all children", new EventHandler(this.OnCheckAllChildrenClick));
            menu.MenuItems.Add("Uncheck all children", new EventHandler(this.OnUncheckAllChildrenClick));
            menu.MenuItems.Add("-");
        ...
        public override void Initialize(DrawArgs drawArgs)
        {
            ...
            try
            {
                foreach (RenderableObject ro in this.m_children)
                {
                    ...
                    {
                        Log.Write(Log.Levels.Error, "ROBJ", string.Format("{0}: {1} ({2})", this.Name, caught.Message, ro.Name));
                    }
                }
            }
            finally
        ...
        public override XmlNode ToXml(XmlDocument worldDoc)
        {
            ...
            showAtStartup.Value = this.IsOn.ToString();
            XmlAttribute showOnlyOneLayer = worldDoc.CreateAttribute("ShowOnlyOneLayer");
            showOnlyOneLayer.Value = this.ShowOnlyOneLayer.ToString();
        ...
        public override void Dispose()
        {
            ...
            try
            {
                ...
                if (this.m_RefreshTimer != null && this.m_RefreshTimer.Enabled) this.m_RefreshTimer.Stop();
            }
            finally
            {
                this.m_childrenRWLock.ReleaseReaderLock();
        ...
        public bool GetReaderLock(int retrys)
        {
            for (int i = 0; i < retrys; i++)
            {
                try
                {
                    this.m_childrenRWLock.AcquireWriterLock(5);
                    return true;
                }
                catch
                {
