    ...
    public partial class WorldWindow : UserControl, IGlobe
    {
        ...
        public World CurrentWorld
        {
            ...
            set
            {
                ...
                {
                    MomentumCamera camera = new MomentumCamera(this.m_World.Position, this.m_World.EquatorialRadius);
                    ...
                    this.layerManagerButton = new LayerManagerButton(
                        Path.Combine(Path.GetDirectoryName(Application.ExecutablePath), @"Data\Icons\Interface\layer-manager2.png"), this.m_World);
                    ...
                    this._menuBar.AddToolsMenuButton(new LatLonMenuButton(Path.GetDirectoryName(Application.ExecutablePath) + "\\Data\\Icons\\Interface\\lat-long.png", this.m_World), 2);
                    this.layerManagerButton.SetPushed(World.Settings.ShowLayerManager);

                    // TODO: Decide how to load grids
                    this.m_World.RenderableObjects.Add(new LatLongGrid(this.m_World));
                }
            }
        }
        ...
        public bool IsRenderDisabled
        {
            get
            {
                return this.m_isRenderDisabled;
            }
            set
            {
                this.m_isRenderDisabled = value;
            }
        }
        ...
        public WorldWindow()
        {
            ...
            try
            {
                ...
                if (!this.DesignMode)
                {
                    ...
                    this.m_FpsTimer.Elapsed += new System.Timers.ElapsedEventHandler(this.m_FpsTimer_Elapsed);
        ...
        private void m_FpsTimer_Elapsed(object sender, System.Timers.ElapsedEventArgs e)
        {
            ...
            this.m_FpsUpdate = true;

            try
            {
                if (World.Settings.ShowFpsGraph)
                {
                    if (!this.m_FpsGraph.Visible)
                    {
                        this.m_FpsGraph.Visible = true;
                    }

                    if (this.m_FrameTimes.Count > World.Settings.FpsFrameCount)
                    {
                        this.m_FrameTimes.RemoveRange(0, this.m_FrameTimes.Count - World.Settings.FpsFrameCount);
                    }

                    this.m_FpsGraph.Size = new Size((int)(this.Width * .5), (int)(this.Height * .1));
                    this.m_FpsGraph.Location = new Point((int)(this.Width * .35), (int)(this.Height * .895));
                    this.m_FpsGraph.Values = (float[]) this.m_FrameTimes.ToArray(typeof(float));
                }
                else
                {
                    if (this.m_FpsGraph.Visible)
                    {
                        this.m_FpsGraph.Visible = false;
                    }
                }
            }
            catch (Exception ex)
        ...
        /// Go to worldwind URI
        /// </summary>
        public void Goto(WorldWindUri uri)
        {
            this.GotoLatLon(
                uri.Latitude.Degrees, uri.Longitude.Degrees, uri.Direction.Degrees,
                uri.Altitude, uri.ViewRange.Degrees, uri.Tilt.Degrees);
            this.drawArgs.WorldCamera.Bank = uri.Bank;
            this.drawArgs.UpperLeftCornerText = uri.ToString();
            this.CurrentWorld.RenderableObjects.Enable(uri.Layer);
        }
        ...
        public void GotoLatLon(double latitude, double longitude, double heading, double altitude, double perpendicularViewRange, double tilt)
        {
            if (!double.IsNaN(perpendicularViewRange))
                altitude = this.m_World.EquatorialRadius * Math.Sin(MathEngine.DegreesToRadians(perpendicularViewRange * 0.5));
            if (altitude < 1)
                altitude = 1;
        ...
        {
            double altitude = this.m_World.EquatorialRadius * Math.Sin(MathEngine.DegreesToRadians(perpendicularViewRange * 0.5));
        ...
        public void OnApplicationIdle(object sender, EventArgs e)
        {
            ...
            try
            {
                if (this.Parent.Focused && !this.Focused) this.Focus();

                while (IsAppStillIdle)
                {
                    if (!World.Settings.AlwaysRenderWindow && this.m_isRenderDisabled && !World.Settings.CameraHasMomentum)
                        return;
            ...
            using (new DirectXProfilerEvent("WorldWindow::Render"))
            {
                ...
                try
                {
                    ...
                    if (this.m_WorkerThread == null)
                    {
                        ...
                        this.m_WorkerThread = new Thread(new ThreadStart(this.WorkerThreadFunc));
                        this.m_WorkerThread.Name = "WorldWindow.WorkerThreadFunc";
                    ...
                    {
                        this.fps = this.frameCounter / (float)(DrawArgs.CurrentFrameStartTicks - this.lastFpsUpdateTime) * PerformanceTimer.TicksPerSecond;
                        this.frameCounter = 0;
                        this.lastFpsUpdateTime = DrawArgs.CurrentFrameStartTicks;
                    }

                    this.m_RootWidget.Render(this.drawArgs);
                    this.m_NewRootWidget.Render(this.drawArgs);
                    ...
                    */
                    ...
                    this._menuBar.Render(this.drawArgs);
                    this.m_FpsGraph.Render(this.drawArgs);

                    if (this.m_World.OnScreenMessages != null)
                    {
                        try
                        {
                            foreach (OnScreenMessage dm in this.m_World.OnScreenMessages)
                            {
                ...
                finally
                {
                    ...
                    if (World.Settings.ShowFpsGraph)
                    {
                        ...
                        this.m_FrameTimes.Add(elapsedMilliSeconds);
                    }
                    this.drawArgs.EndRender();
                }

                this.drawArgs.UpdateMouseCursor(this);
            }
        }
        ...
        protected void RenderPositionInfo()
        {
            ...
            string captionText = this._caption;
            ...
            if (World.Settings.ShowPosition)
            {
                ...
                if (this.drawArgs.WorldCamera.AltitudeAboveTerrain < 300000)
                {
                    double terrainElevation = this.drawArgs.WorldCamera.TerrainElevation;
                    if (World.Settings.DisplayUnits == Units.Metric)
                    {
            ...
            if (this.showDiagnosticInfo)
            {
                ...
                captionText +=
                    ...
                    "\nRO: " + this.m_World.RenderableObjects.Count.ToString("f0") +
            ...
            if (this._menuBar.IsActive)
            {
                this.positionAlpha -= positionAlphaStep;
                if (this.positionAlpha < this.positionAlphaMin)
                {
                    this.positionAlpha = this.positionAlphaMin;
                }
            }
            else
            {
                this.positionAlpha += positionAlphaStep;
                if (this.positionAlpha > this.positionAlphaMax) this.positionAlpha = this.positionAlphaMax;
            }

            int positionBackColor = this.positionAlpha << 24;
            int positionForeColor = (int)((uint)(this.positionAlpha << 24) + 0xffffffu);
        ...
        protected override void OnMouseWheel(MouseEventArgs e)
        {
            ...
            {
                if (this.m_NewRootWidget != null)
                {
                    try
                    {
                        this.m_NewRootWidget.OnMouseWheel(e);
                    }
                    finally
                    {
                    }
        ...
        protected override void OnKeyDown(KeyEventArgs e)
        {
            ...
            try
            {
                e.Handled = this.HandleKeyDown(e);
                base.OnKeyDown(e);
            }
            catch (Exception caught)
            {
        ...
        protected override void OnKeyPress(KeyPressEventArgs e)
        {
            ...
            if (this.m_RootWidget != null)
            {
                bool handled = this.m_RootWidget.OnKeyPress(e);
                e.Handled = handled;
            }
            if (this.m_NewRootWidget != null)
            {
                bool handled = this.m_NewRootWidget.OnKeyPress(e);
                e.Handled = handled;
            }
            base.OnKeyPress(e);
        }
        ...
        public bool HandleKeyUp(KeyEventArgs e)
        {
            ...
            bool handled = this.m_RootWidget.OnKeyUp(e);
            if (handled)
            {
                e.Handled = handled;
                return handled;
            }
            handled = this.m_NewRootWidget.OnKeyUp(e);
            if (handled)
            {
                e.Handled = handled;
                return handled;
        ...
        protected override void OnMouseDown(MouseEventArgs e)
        {
            ...
            this.mouseDownStartPosition.X = e.X;
            this.mouseDownStartPosition.Y = e.Y;


            try
            {
                bool handled = false;
                handled = this.m_RootWidget.OnMouseDown(e);

                if (!handled)
                {
                    handled = this.m_NewRootWidget.OnMouseDown(e);
                }
        ...
        protected override void OnMouseUp(MouseEventArgs e)
        {
            ...
            try
            {
                ...
                handled = this.m_RootWidget.OnMouseUp(e);

                if (!handled)
                {
                    handled = this.m_NewRootWidget.OnMouseUp(e);
                }

                if (!handled)
                {
                    // Mouse must have been clicked outside our window and released on us, ignore
                    if (this.mouseDownStartPosition == Point.Empty)
                        return;

                    this.mouseDownStartPosition = Point.Empty;
                    ...
                    if (this.isDoubleClick)
                    {
                        ...
                        if (e.Button == MouseButtons.Left)
                        {
                            this.drawArgs.WorldCamera.Zoom(World.Settings.CameraDoubleClickZoomFactor);
                        }
                    ...
                    else
                    {
                        if (e.Button == MouseButtons.Left)
                        {
                            ...
                            {
                                if (!this.m_World.PerformSelectionAction(this.drawArgs))
                                {
                                    ...
                                    if (this.drawArgs.WorldCamera.Altitude < 60e3)
                                        this.drawArgs.WorldCamera.PickingRayIntersectionWithTerrain(
                                         ...
                                         DrawArgs.LastMousePosition.Y,
                                         out targetLatitude,
                                         out targetLongitude, this.m_World);
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            try
            {
                ...
                if (!this.isMouseDragging)
                {
                    handled = this.m_RootWidget.OnMouseMove(e);

                    if (!handled)
                    {
                        handled = this.m_NewRootWidget.OnMouseMove(e);
                    }
                }
                ...
                if (!handled)
                {
                    ...
                    float deltaXNormalized = (float)deltaX / this.drawArgs.screenWidth;
                    float deltaYNormalized = (float)deltaY / this.drawArgs.screenHeight;
        ...
        protected override void OnMouseLeave(EventArgs e)
        {
            ...
            if (this._menuBar != null)
                ...
                this._menuBar.OnMouseMove(new MouseEventArgs(MouseButtons.None, 0, -1, -1, 0));
            base.OnMouseLeave(e);
        }
        ...
        private void WorkerThreadFunc()
        {
            ...
            while (this.m_WorkerThreadRunning)
            {
                try
                {
                    if (World.Settings.UseBelowNormalPriorityUpdateThread && this.m_WorkerThread.Priority == ThreadPriority.Normal)
                    {
                        this.m_WorkerThread.Priority = ThreadPriority.BelowNormal;
                    }
                    else if (!World.Settings.UseBelowNormalPriorityUpdateThread && this.m_WorkerThread.Priority == ThreadPriority.BelowNormal)
                    {
                        this.m_WorkerThread.Priority = ThreadPriority.Normal;
                    }
        ...
        {
            this.m_World.OnScreenMessages = messages;
        }
