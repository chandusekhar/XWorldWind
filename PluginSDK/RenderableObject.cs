...
namespace WorldWind
{
	...
	public abstract class RenderableObject : IRenderable, IComparable
	{
		...
		protected System.Windows.Forms.Form m_propertyBrowser;
		...
		/// <summary>
		/// Initializes a new instance of the <see cref= "T:WorldWind.RenderableObject"/> class.
		/// </summary>
        ...
        {
            return worldDoc.CreateComment("ERROR: RenderableObject \"" + this.Name + "\" does not implement the ToXML method and can not be serialized.");
        }
		...
		public virtual string Thumbnail
		{
			...
			set
			{
                this.m_thumbnail = ImageHelper.FindResource(value);
			}
		}
		...
		public virtual Image ThumbnailImage
		{
			get
			{
				if(this.m_thumbnailImage==null)
				{
					...
					try
					{
						if(File.Exists(this.m_thumbnail)) this.m_thumbnailImage = ImageHelper.LoadImage(this.m_thumbnail);
					}
					catch {}
				}
				return this.m_thumbnailImage;
			}
		}
		...
		public Image IconImage
		{
			get
			{
				...
				{
					if(this.m_iconImagePath==null)
						return null;
					try
					{
						if(File.Exists(this.m_iconImagePath)) this.m_iconImage = ImageHelper.LoadImage(this.m_iconImagePath);
					}
					catch {}
				}
		...
		{
			menu.MenuItems.Add("Goto", new EventHandler(this.OnGotoClick));
			menu.MenuItems.Add("Properties...", new EventHandler(this.OnPropertiesClick));
			menu.MenuItems.Add("Info...", new EventHandler(this.OnInfoClick));
			menu.MenuItems.Add("Delete...", new EventHandler(this.OnDeleteClick));
			menu.MenuItems.Add("Reload Shader", new EventHandler(this.OnReloadShaderClick));
            menu.MenuItems.Add("Save as...", new EventHandler(this.OnSaveAsClick));
            menu.MenuItems.Add("View xml...", new EventHandler(this.OnViewXmlClick));

			if(this.dbfPath != "")
				menu.MenuItems.Add("Dbf Info", new EventHandler(this.OnDbfInfo));
		...
		{			
			ShapeFileInfoDlg sfid = new ShapeFileInfoDlg(this.dbfPath, this.dbfIsInZip);
			sfid.Show();	
		}
		...
		{
			LayerManagerItemInfo lmii = new LayerManagerItemInfo(this.MetaData);
			lmii.ShowDialog();
		}
		...
		{
			if(this.m_propertyBrowser!=null) this.m_propertyBrowser.Dispose();

            this.m_propertyBrowser = new PropertyBrowser(this);
            this.m_propertyBrowser.Show();
		}
