...
namespace WorldWind
{
	...
	{
		ProjectedVectorTile[] m_rootTiles;
		double m_lzts = 36.0;
		...
		{
			get{ return (Polygon[]) this.m_polygons.ToArray(typeof(Polygon)); }
		}

		public LineString[] LineStrings
		{
			get{ return (LineString[]) this.m_lineStrings.ToArray(typeof(LineString)); }
		}

		public ProjectedVectorRenderer(World parentWorld)
		{
            this.World = parentWorld;
            this.UpdateRootTiles();
		}
		...
		public void Add(LineString lineString)
		{
            ...
            this.m_lineStrings.Add(lineString);
            this.LastUpdate = System.DateTime.Now;
		}

		public void Update(DrawArgs drawArgs)
		{
			for(int i = 0; i < this.m_lineStrings.Count; i++)
			{
				LineString lineString = (LineString) this.m_lineStrings[i];
				if(lineString.Remove)
				{
                    this.m_lineStrings.RemoveAt(i);
                    this.LastUpdate = System.DateTime.Now;
					i--;
				}
			...
			{
				Polygon polygon = (Polygon) this.m_polygons[i];
				if(polygon.Remove)
				{
                    this.m_polygons.RemoveAt(i);
                    this.LastUpdate = System.DateTime.Now;
					i--;
				}
			...
			foreach(ProjectedVectorTile tile in this.m_rootTiles)
				tile.Update(drawArgs);
		}

		private static bool IsRenderableVisible(RenderableObject renderable)
		{
			if(!renderable.IsOn)
			{
				return false;
		...
		{
			drawArgs.device.Clear(SharpDX.Direct3D9.ClearFlags.ZBuffer, 0, 1.0f, 0);
