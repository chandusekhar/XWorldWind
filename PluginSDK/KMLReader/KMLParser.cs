...
namespace WorldWind.KMLReader
{
    public class KMLParser
    {
        ...
        public void ReadKML(string kml, Icons layer, string path)
        {
            ...
            if (inNode != null) this.ParseRenderables(inNode, layer, path);
        }
        ...
        public void Cleanup()
        {
            ...
            {
                foreach (RenderableObject ro in this.m_layer.ChildObjects)
                {
                    ro.Dispose();
                }

                this.m_layer.ChildObjects.Clear();
            }

            foreach (Object bit in this.bitmapCache)
            {
            ...
            foreach (KMLNetworkLink netLink in this.networkLinks)
            {
                netLink.Dispose();
            }
        ...
        private void ParseStyles(XmlDocument doc, string KmlPath)
        {
            ...
            foreach (XmlNode xstyle in styles)
            {
                ...
                if (this.iconStyles.ContainsKey(name))
                    continue;

                Style style = this.GetStyle(xstyle, new Style(), KmlPath);
                if (style != null) this.iconStyles.Add(name, style);
            ...
            foreach (XmlNode stylemap in stylemaps)
            {
                ...
                XmlNode stylemapNode = stylemap.SelectSingleNode("Pair[key=\"normal\"]/styleUrl");
                ...
                Style style = this.GetStyle(normalNode, new Style(), KmlPath);
                if (style != null) this.iconStyles.Add(name, style);
            }
        ...
        private void ParseRenderables(XmlNode inNode, Icons layer, string KmlPath)
        {
            ...
            // Parse all Folders
            this.ParseFolders(inNode, layer, KmlPath);

            // Parse NetworkLinks
            this.ParseNetworkLinks(inNode, layer);

            // Parse GroundOverlays
            this.ParseGroundOverlays(inNode, layer);

            //Parse ScreenOverlays
            this.ParseScreenOverlays(inNode, layer);

            // Parse Placemarks
            this.ParsePlacemarks(inNode, layer, KmlPath);

            // Parse LineStrings
            this.ParseLineStrings(inNode, layer);

            // Parse Polygons
            this.ParsePolygons(inNode, layer);

            // Parse MultiGeometry
            this.ParseMultiGeometry(inNode, layer);
        ...
        private void ParseFolders(XmlNode inNode, Icons layer, string KmlPath)
        {
            ...
            foreach (XmlNode node in folders)
            {
                try
                {
                    ...
                    this.ParseRenderables(node, folder, KmlPath);
                }
                catch (Exception ex)
        ...
        private void ParsePlacemarks(XmlNode inNode, Icons layer, string KmlPath)
        {
            ...
            foreach (XmlNode node in placemarks)
            {
                try
                {
                    ...
                    LLA lla = this.ParseCoordinate(loc);
                    ...
                    Style style = this.LocateStyle(node, KmlPath);
                    ...
                    if (update != null)
                    {
                        ...
                        if (style != null)
                        {
                            update.Height = Double.IsNaN(style.NormalScale) ? IconSizeConstant : (int)(style.NormalScale * Math.Min(((Bitmap) this.bitmapCache[style.NormalIcon]).Height, IconSizeConstant));
                            update.Width = Double.IsNaN(style.NormalScale) ? IconSizeConstant : (int)(style.NormalScale * Math.Min(((Bitmap) this.bitmapCache[style.NormalIcon]).Width, IconSizeConstant));
                    ...
                    else
                    {
                        ...
                        if (style != null)
                        {
                            this.CreateIcon(layer, name, desc, uri, lla.lat, lla.lon, lla.alt, style, bRotated, rotation, bExtrude);
                        }
                        else
                        {
                            ...
                            {
                                if (!this.bitmapCache.Contains(pal3Path)) this.bitmapCache.Add(pal3Path, (Bitmap)Image.FromFile(pal3Path));
                                Style pinStyle = new Style(this.GetSubImage(new Style(pal3Path), 448, 64, 64, 64));

                                this.CreateIcon(layer, name, desc, uri, lla.lat, lla.lon, lla.alt, pinStyle, bRotated, rotation, bExtrude);
                            }
                        }
                    }
                }
        ...
        private void ParseLineStrings(XmlNode inNode, Icons layer)
        {
            ...
            foreach (XmlNode node in lineStrings)
            {
                ...
                if (styleUrlNode != null)
                {
                    ...
                    style = (Style) this.iconStyles[styleUrlKey];
                }
                else
                {
                    XmlNode styleNode = node.SelectSingleNode("Style");
                    if (styleNode != null)
                        style = this.GetStyle(styleNode, new Style(), "");
                }
                ...
                if (outerRingNode != null)
                {
                    ...
                    Point3d[] points = this.ParseCoordinates(outerRingNode);
                    LineFeature line = new LineFeature(name, DrawArgs.CurrentWorldStatic, points, Color.FromArgb(style.LineStyle.Color.Color));

                    XmlNode altitudeModeNode = node.SelectSingleNode("LineString/altitudeMode");
                    line.AltitudeMode = this.GetAltitudeMode(altitudeModeNode);
                    ...
                    if (extrude)
                    {
                        ...
                        {
                            line.PolygonColor = Color.FromArgb(style.PolyStyle.Color.Color);
                        }
                    }
        ...
        private void ParseMultiGeometry(XmlNode inNode, Icons layer)
        {
            ...
            foreach (XmlNode placemarkNode in placemarkNodes)
            {
                ...
                foreach (XmlNode lineStringNode in lineStringNodes)
                {
                    ...
                    Point3d[] points = this.ParseCoordinates(coordinateNode);

                    XmlNode altitudeModeNode = lineStringNode.SelectSingleNode("altitudeMode");
                    AltitudeMode altitudeMode = this.GetAltitudeMode(altitudeModeNode);
                    ...
                    if (points != null && points.Length > 0)
                    {
                        LineFeature line = new LineFeature(
                            ...
                            DrawArgs.CurrentWorldStatic,
                            points,
                            Color.FromArgb(style.LineStyle.Color.Color)
                            );
                ...
                foreach (XmlNode polygonNode in polygonNodes)
                {
                    ...
                    if (innerRingNodes != null)
                    {
                        ...
                        {
                            Point3d[] points = this.ParseCoordinates(innerRingNodes[i]);
                    ...
                    if (outerRing != null)
                    {
                        PolygonFeature polygonFeature = new PolygonFeature(
                            ...
                            outerRing,
                            innerRings,
                            (style.PolyStyle.Color != null ? Color.FromArgb(style.PolyStyle.Color.Color) : Color.Yellow)
                            );
                        ...
                        if (style.LineStyle.Color != null)
                            polygonFeature.OutlineColor = Color.FromArgb(style.LineStyle.Color.Color);
            ...
            foreach (XmlNode node in polygons)
            {
                ...
                if (outerRing != null)
                {
                    PolygonFeature polygonFeature = new PolygonFeature(
                        ...
                        outerRing,
                        innerRings,
                        Color.FromArgb(style.PolyStyle.Color.Color));
            ...
            try
            {
                ...
                if (hrefNode != null)
                {
                    ...
                    if (filename.StartsWith("root://"))													// Use palette bitmap
                    {
                        ...
                        {
                            bPalette = true;
                            overStyle.NormalIcon = this.GetDiskImage(filename);
                        }
                    }
                    else if (filename.StartsWith("http://"))											// Use bitmap from the internet
                    {
                        overStyle.NormalIcon = this.GetWebImage(filename);
                    }
                    else if (File.Exists(Path.Combine(Path.GetDirectoryName(KmlPath), filename)))		// Use a file from disk
                    {
                        overStyle.NormalIcon = this.GetDiskImage(Path.Combine(Path.GetDirectoryName(KmlPath), filename));
                    }
                    else if (File.Exists(Path.Combine(KmlDirectory, filename)))					// Use a file from disk
                    {
                        overStyle.NormalIcon = this.GetDiskImage(Path.Combine(KmlDirectory, filename));
                    }
                }
                ...
                if (wNode != null && hNode != null)
                {
                    ...
                    if (bPalette)
                        overStyle.NormalIcon = this.GetSubImage(overStyle, x * 2, y * 2, w * 2, h * 2);
                    else
                        overStyle.NormalIcon = this.GetSubImage(overStyle, x, y, w, h);
                }
                ...
                if (balloonStyleNode != null)
                {
                    ...
                    if (balloonTextNode != null)
                    {
                        ...
                        if (textNodeColor != null)
                            textElement.TextColor = new ColorElement(this.ParseColor(textNodeColor.InnerText));
                    ...
                    if (balloonTextColorNode != null)
                        balloonStyle.TextColor = new ColorElement(this.ParseColor(balloonTextColorNode.InnerText));

                    XmlNode balloonColorNode = balloonStyleNode.SelectSingleNode("color");
                    if (balloonColorNode != null)
                        balloonStyle.Color = new ColorElement(this.ParseColor(balloonColorNode.InnerText));
                    ...
                    if (iconElementNode != null)
                    {
                        ...
                        if (iconElementHrefNode != null)
                        {
                            ...
                            if (filename.StartsWith("root://"))													// Use palette bitmap
                            {
                                ...
                                {
                                    bPalette = true;
                                    iconElement.href = this.GetDiskImage(filename);
                                }
                            }
                            else if (filename.StartsWith("http://"))											// Use bitmap from the internet
                            {
                                iconElement.href = this.GetWebImage(filename);
                            }
                            else if (File.Exists(Path.Combine(Path.GetDirectoryName(KmlPath), filename)))		// Use a file from disk
                            {
                                iconElement.href = this.GetDiskImage(Path.Combine(Path.GetDirectoryName(KmlPath), filename));
                            }
                            else if (File.Exists(Path.Combine(KmlDirectory, filename)))					// Use a file from disk
                            {
                                iconElement.href = this.GetDiskImage(Path.Combine(KmlDirectory, filename));
                            }
                        }
                        ...
                        if (iconElementWNode != null && iconElementHNode != null)
                        {
                            ...
                            if (bPalette)
                                iconElement.href = this.GetSubImage(overStyle, x * 2, y * 2, w * 2, h * 2);
                            else
                                iconElement.href = this.GetSubImage(overStyle, x, y, w, h);
                        ...
                        if (iconStyleColorNode != null)
                            iconStyle.Color = new ColorElement(this.ParseColor(iconStyleColorNode.InnerText));
                ...
                if (labelStyleNode != null)
                {
                    ...
                    if (labelColorNode != null)
                        labelStyle.Color = new ColorElement(this.ParseColor(labelColorNode.InnerText));
                ...
                if (lineStyleNode != null)
                {
                    ...
                    if (lineColorNode != null)
                        lineStyle.Color = new ColorElement(this.ParseColor(lineColorNode.InnerText));
                ...
                if (polyStyleNode != null)
                {
                    ...
                    if (polyColorNode != null)
                        polyStyle.Color = new ColorElement(this.ParseColor(polyColorNode.InnerText));
        ...
        private int ParseColor(string s)
        {
            ...
            return Color.FromArgb(
                int.Parse(a, NumberStyles.HexNumber),
                int.Parse(r, NumberStyles.HexNumber),
                int.Parse(g, NumberStyles.HexNumber),
                int.Parse(b, NumberStyles.HexNumber)).ToArgb();
        ...
        private string GetSubImage(Style style, int x, int y, int w, int h)
        {
            ...
            if (this.bitmapCache.ContainsKey(key))
                return key;
            ...
            if (style.NormalIcon != null && this.bitmapCache.Contains(style.NormalIcon))
            {
                Bitmap bit = ((Bitmap) this.bitmapCache[style.NormalIcon]);
                RectangleF sourceRect = new RectangleF(x, bit.Height - y - h, w, h);
                graphics.DrawImage((Bitmap) this.bitmapCache[style.NormalIcon], destinationRect, sourceRect, GraphicsUnit.Pixel);
                graphics.Flush();

                // Cache the generated bitmap
                this.bitmapCache.Add(key, outImage);
        ...
        private string GetDiskImage(string filename)
        {
            ...
            this.bitmapCache.Add(filename, bit);
        ...
        private string GetWebImage(string filename)
        {
            ...
            Bitmap bit = (Bitmap)Image.FromStream(myDownload.ContentStream);
        ...
        private Style LocateStyle(XmlNode node, string KmlPath)
        {
            ...
            if (styleNode != null)
            {
                ...
                style = (Style) this.iconStyles[styleUrl];
            }
            ...
            {
                if (style != null)
                    style = this.GetStyle(styleNode, style, KmlPath);
                else
                    style = this.GetStyle(styleNode, new Style(), KmlPath);
            }
            return style;
        }
        #endregion
            ...
            Style style, bool bRotated, float rotation, bool bExtrude)
        {
            ...
            if (style.NormalIcon != null && this.bitmapCache.Contains(style.NormalIcon))
            {
                ic.Image = (Bitmap) this.bitmapCache[style.NormalIcon];
                ic.Height = Double.IsNaN(style.NormalScale) ? IconSizeConstant : (int)(style.NormalScale * Math.Min(((Bitmap) this.bitmapCache[style.NormalIcon]).Height, IconSizeConstant));
                ic.Width = Double.IsNaN(style.NormalScale) ? IconSizeConstant : (int)(style.NormalScale * Math.Min(((Bitmap) this.bitmapCache[style.NormalIcon]).Width, IconSizeConstant));
            }
        ...
        private static void RemoveUnusedIcons(Icons layer)
        {
            ...
            foreach (RenderableObject ro in VFD)
            {
                layer.Remove(ro);
                ro.Dispose();
            }
        ...
        class Style
        {
            ...
            public static Color HexToColor(string hexColor)
            {
                ...
                {
                    int ai
                        = Int32.Parse(a, NumberStyles.HexNumber);
                    int ri
