...
namespace WorldWind.KMLReader
{
    ...
    {
        static string KmlDirectory = Path.Combine(Path.GetDirectoryName(Application.ExecutablePath), "kml");
        ...
        public string LoadKML(string filename)
        {
            ...
            {
                bool shouldReturn;
                string ExtractedKMLPath = this.ExtractKMZ(filename, out shouldReturn);
                if (shouldReturn)
                    return null;
                this.m_realkmlpath = ExtractedKMLPath;
                return this.LoadKMLFile(ExtractedKMLPath);
            }
            else
            {
                this.m_realkmlpath = filename;
                return this.LoadKMLFile(filename);
            }
        }
        ...
        internal string ExtractKMZ(string filename, out bool bError)
        {
            ...
            {
                ExtractedKMLPath = this.GetKMLFromDirectory(ExtractPath);
        ...
        private string GetKMLFromDirectory(string ExtractPath)
        {
            ...
            {
                string tempPath = this.GetKMLFromDirectory(folder);
                if (tempPath != null)
                    return tempPath;
            }
        ...
        private string LoadKMLFile(string KMLPath)
        {
            ...
            try
            {
                StreamReader sr = new StreamReader(KMLPath);
