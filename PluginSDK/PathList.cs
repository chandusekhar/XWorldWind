...
namespace WorldWind
{
	...
	public class PathList : RenderableObjectList
	{
		...
		/// <summary>
		/// Initializes a new instance of the <see cref= "T:WorldWind.PathList"/> class.
		/// </summary>
		...
		{
			DirectoryInfo pathsDir = new DirectoryInfo(this.pathsDirectoryPath);
			...
			if(indexFileInfo.Exists && dataFileInfo.Exists) 
			{
				...
				{
                    this.dataArchiveReader = new BinaryReader(new BufferedStream(dataFileInfo.OpenRead()), System.Text.Encoding.ASCII);
					...
					for(int i = 0; i < count; i++) 
					{
						...
						TerrainPath tp = new TerrainPath(
							fileName, this.parentWorld, this.minDisplayAltitude, this.maxDisplayAltitude, this.dataArchiveReader,
							offset,
							0,
							north,
							south,
							east,
							west,
							(float) this.altitude, this.color, this.terrainAccessor);
			...
			else
			{
				...
				{
                    this.Add(new TerrainPath(file.Name, this.parentWorld, this.minDisplayAltitude, this.maxDisplayAltitude,
						file.FullName, (float) this.altitude, this.color, this.terrainAccessor));
				}
			}
		...
		public override void Dispose()
		{
			try
			{
				...
				if (this.dataArchiveReader!=null) this.dataArchiveReader.Close();
			}
			finally
			{
				base.Dispose();
