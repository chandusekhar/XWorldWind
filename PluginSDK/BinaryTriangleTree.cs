...
namespace WorldWind
{
	...
	public class BinaryTriangleTree
	{
			...
			double LayerRadius)
		{
			...
			this.sw = (short)(this.nw + (short)(this._vertexDensity * this._verticeDensity));
			this.ne = (short)(this.nw + (short)(this._vertexDensity));
			this.se = (short)(this.sw + (short)(this._vertexDensity));
		...
		public void BuildTree(int MaxVariance)
		{
            ...
            this.BuildFace(0);
            this.BuildFace(1);
			this._terrainFaceCount = this.FaceCount();
		}
		...
		private void BuildFace(short f)
		{
			...
			{
                this.BuildFace(this._treeList[f].lc);
                this.BuildFace(this._treeList[f].rc);
			}
			else
			{
				if(this.FaceVariance(this._treeList[f].i1, this._treeList[f].i2, this._treeList[f].i3) > this._maxVariance)
		...
		private void SplitFace(short f) 
		{
			if(this._treeList[f].bn != -1)		// Check bottom neighbour first
			{
				...
				{
                    this.SplitFace(this._treeList[f].bn);	// split bottom neighbour
				}

                this.SplitFaceForReal(f);					// now split face and bot. neighbour
                this.SplitFaceForReal(this._treeList[f].bn);
		...
		private void SplitFaceForReal(short f)
		{
			...
			short mid_hyp_indice = this.MidHypVerticeIndice(this._treeList[f].i1, this._treeList[f].i3);
		...
		private double FaceVariance(short v1, short v2, short v3)
		{
			...
			{
				// find vertice in middle of hypothenuse
				short mid_hyp_indice = this.MidHypVerticeIndice(v1, v3);
				...
				MaxVar = Math.Max(MaxVar, this.FaceVariance(v2, mid_hyp_indice, v1));
				MaxVar = Math.Max(MaxVar, this.FaceVariance(v3, mid_hyp_indice, v2));
			}
			return MaxVar;
		}
		...
		public void BuildIndices()
		{
			...
			if(this._margin != 0) 
			{
				...
				{
					if(this.VerticeIsUsedInTerrain(i2))
					{
						this.Indices[idx] = i1;
