...
namespace WorldWind
{
	...
	public class DownloadableImageFromIconSet : RenderableObject
	{
		...
		/// <summary>
		/// Initializes a new instance of the <see cref= "T:WorldWind.DownloadableImageFromIconSet"/> class.
		/// </summary>
			...
			float west, float south, float east, float north, string imageUrl, string saveTexturePath, string iconFilePath, int iconSize, string caption)
		{
			...
			DownloadableIcon di = new DownloadableIcon(name, this.m_ParentWorld, 
				this.layerRadius - (float) this.m_ParentWorld.EquatorialRadius + altitude, west, south, east, north,
		...
		public void LoadDownloadableIcon(string name)
		{
			...
			if(di != null)
			{
				...
				di.DownLoadImage(this.drawArgs);
	...
	public class DownloadableIcon : RenderableObject
	{
		...
		static int bottomLeftTextColor = Color.Cyan.ToArgb();
		...
		{
			get{ return this.centerLon; }
		}
		public string IconFilePath
		{
			get{ return this.iconFilePath; }
			set{ this.iconFilePath = value; }
		}
		public float North
		{
		...
		{
			get{ return this.saveTexturePath; }
		}
		...
		public override void Initialize(DrawArgs drawArgs)
		{
			...
			for (int i=0; i< this.progressBarOutline.Length; i++) this.progressBarOutline[i].Z = 0.5f;
			for (int i=0; i< this.progressBar.Length; i++) this.progressBar[i].Z = 0.5f;
		...
		public override bool PerformSelectionAction(DrawArgs drawArgs)
		{
			return this.DownLoadImage(drawArgs);
		}
		...
		public void DownloadImage()
		{
			try
			{
                ...
                using( WebDownload dl = new WebDownload(this.imageUrl))
				{
					dl.ProgressCallback += new DownloadProgressHandler(this.UpdateProgress);
					dl.DownloadMemory();

					this.downloadState = DownloadState.Converting;
					ImageHelper.ConvertToDxt1(dl.ContentStream, this.saveTexturePath);
				}
				...
				{
					this.imageLayer = new ImageLayer(this.Name, this.m_ParentWorld,
						this.layerRadius - (float) this.m_ParentWorld.EquatorialRadius, this.saveTexturePath, this.south, this.north, this.west, this.east, 255, /*this.terrainInfo*/null);
				}
		...
		{
            this.downloadProgress = total>0 ? (float)current / total : 0;
		}
