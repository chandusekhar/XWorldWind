...
namespace WorldWind.Terrain
{
	...
	public class TerrainDownloadRequest : global::GeoSpatialDownloadRequest
	{
		...
		public TerrainDownloadRequest(TerrainTile tile, TerrainTileService owner, int row, int col, int targetLevel) : base(owner)
		{
            ...
            this.download.Url = String.Format(CultureInfo.InvariantCulture,
				"{0}?T={1}&L={2}&X={3}&Y={4}",
				owner.ServerUrl,
				owner.DataSet,
				targetLevel, col, row );
		...
		public override float West
		{
			get 
			{
				return (float) this.TerrainTile.West;
			}
		}
		...
		public override float East
		{
			get 
			{
				return (float) this.TerrainTile.East;
			}
		}
		...
		public override float North
		{
			get
			{
				return (float) this.TerrainTile.North;
			}
		}
		...
		public override float South
		{
			get 
			{
				return (float) this.TerrainTile.South;
			}
		}
		...
		{
            if (this.download.ContentType == ContentTypeZip)
            {
                string compressedPath = this.download.SavedFilePath + ".zip";

                DirectoryInfo tempDirectory = new DirectoryInfo(Path.GetDirectoryName(compressedPath) + "\\" + DateTime.Now.Ticks.ToString());
                tempDirectory.Create();

                string tempFullPath = Path.Combine(tempDirectory.FullName, Path.GetFileNameWithoutExtension(this.download.SavedFilePath));
                ...
                File.Move(this.download.SavedFilePath, compressedPath);
                ...
                File.Move(tempFullPath, this.SaveFilePath);
            ...
            else if (this.download.ContentType == ContentType7z || this.download.ContentType == ContentTypeXCompressed) 
			{
				// Decompress 7z
				string compressedPath = this.download.SavedFilePath + ".7z";

				// Parent of destination
				string tempDirectory = Path.GetDirectoryName(Path.GetDirectoryName(this.download.SavedFilePath));
				string tempFullPath = Path.Combine(tempDirectory, Path.GetFileNameWithoutExtension(this.download.SavedFilePath ) );
		...
		public void DownloadInForeground()
		{
			try
			{
                this.download.DownloadFile(this.download.SavedFilePath);
                this.ProcessFile();
			}
			catch(FileNotFoundException)
			{
		...
		void FlagBadTile()
		{
			...
			using( Stream flagFile = File.Create(this.SaveFilePath) )
			{
			}
		}
