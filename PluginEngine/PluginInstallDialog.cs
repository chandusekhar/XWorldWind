...
namespace WorldWind.PluginEngine
{
	...
	/// </summary>
	public class PluginInstallDialog : Form
	{
...
that you may be assured of a safe and productive World Wind experience.";
			...
			try
			{
                ...
                this.url.Text = this.url.Text.Trim();

				if(this.IsWeb)
                    this.InstallFromUrl(new Uri(this.url.Text));
				else if(this.IsFile)
                    this.InstallFromFile(this.url.Text);
				else
				{
		...
		bool IsFile
		{
			get
			{
				return File.Exists(this.url.Text);
			}
		}
		...
		bool IsWeb
		{
			get
			{
				return this.url.Text.ToLower().StartsWith("http://");
			}
		}
		...
		void InstallFromFile( string pluginPath )
		{
			...
			string destPath = this.GetDestinationPath( fileName );
			if(destPath == null)
				return;
		...
		{
			string directory = Path.Combine(this.m_compiler.PluginRootDirectory, Path.GetFileNameWithoutExtension(fileName));
		...
		private void buttonBrowse_Click(object sender, EventArgs e)
		{
			...
			if(this.IsFile)
				of.FileName = this.url.Text;
			if(of.ShowDialog()!=DialogResult.OK)
				return;
            this.url.Text = of.FileName;
		}
	}
}
