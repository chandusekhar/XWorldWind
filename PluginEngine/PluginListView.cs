...
namespace WorldWind.PluginEngine
{
	...
	public class PluginListView : ListView
	{
			...
			switch(m.Msg)
			{
				case WM_REFLECT | WM_DRAWITEM:
				{
					...
					using( Graphics g = Graphics.FromHdc(dis.hdc) )
					{
						...
						PluginListItem item = (PluginListItem) this.Items[dis.itemID];
						DrawItemEventArgs e = new DrawItemEventArgs(g,this.Font,r,dis.itemID,d);
                        this.OnDrawItem(e, item);
		...
		protected override void OnMouseUp(MouseEventArgs e)
		{
			...
			Message message = Message.Create(this.Handle, LVM_SUBITEMHITTEST, IntPtr.Zero, pointer);
			...
			if(hitInfo.iItem >=0 && hitInfo.iSubItem == 1)
			{
				...
				PluginListItem item = (PluginListItem) this.Items[hitInfo.iItem];
		...
		protected void OnDrawItem( DrawItemEventArgs e, PluginListItem item )
		{
			...
			if(this.imageList==null) this.imageList = ((PluginDialog) this.Parent).ImageList;
			if(this.imageList!=null)
			{
				int imageIndex = item.PluginInfo.IsCurrentlyLoaded ? 0 : 1;
                this.imageList.Draw(e.Graphics, e.Bounds.Left+2, e.Bounds.Top+1, imageIndex);
			}

			// Name
			Rectangle bounds = Rectangle.FromLTRB(e.Bounds.Left+imageWidth, 
				e.Bounds.Top, e.Bounds.Left+ this.Columns[0].Width, e.Bounds.Bottom);
			...
			bounds = Rectangle.FromLTRB(bounds.Right+1, 
				bounds.Top, bounds.Right+ this.Columns[1].Width+1, bounds.Bottom-1);
