...
namespace WorldWind.PluginEngine
{
	...
	/// </summary>
	public class PluginDialog : Form
	{
		...
		void AddPluginList()
		{
			...
			foreach (PluginInfo pi in this.compiler.Plugins) 
			{
		...
		{
			foreach(PluginListItem pi in this.listView.SelectedItems) this.PluginUnload(pi);
		...
		{
			foreach(PluginListItem pi in this.listView.SelectedItems) this.PluginLoad(pi);
            this.listView.Invalidate();
            this.UpdateUIStates();
		}
		...
		private void PluginLoad(PluginListItem pi)
		{
			try
			{
                this.compiler.Load(pi.PluginInfo);
			}
		...
		public void PluginUnload(PluginListItem pi)
		{
			try
			{
                this.compiler.Unload(pi.PluginInfo);
			}
			catch(Exception caught)
			{
				MessageBox.Show("The error was:\n\n" + caught.Message, pi.Name + 
		...
		void UpdateUIStates()
		{
			bool isItemSelected = this.listView.SelectedItems.Count > 0;
            this.buttonUninstall.Enabled = isItemSelected;
			if(!isItemSelected)
			{
			...
			PluginListItem item = (PluginListItem) this.listView.SelectedItems[0];
            this.buttonLoad.Enabled = !item.PluginInfo.IsCurrentlyLoaded;
            this.buttonUnload.Enabled = item.PluginInfo.IsCurrentlyLoaded;
		}
		...
		private void listView_SelectedIndexChanged(object sender, EventArgs e)
		{
            ...
            this.description.Text = item.PluginInfo.Description;
            this.developer.Text = item.PluginInfo.Developer;
            this.webSite.Text = item.PluginInfo.WebSite;
		}

		private void webSite_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
		{
			if(this.webSite.Text == null || this.webSite.Text.Length<=0)
				return;

			Process.Start(this.webSite.Text);
		}

		private void buttonInstall_Click(object sender, EventArgs e)
		{
			Form installDialog = new PluginInstallDialog(this.compiler);
			installDialog.Icon = this.Icon;
			installDialog.ShowDialog();

			// Rescan for plugins
            this.compiler.FindPlugins();
            this.AddPluginList();
		}
		...
		{
			foreach(PluginListItem pi in this.listView.SelectedItems)
			{
				...
				try
				{
                    this.compiler.Uninstall( pi.PluginInfo );

					// Remove if from the plugin list
                    this.listView.Items.Remove(pi);
				}
				catch(Exception caught)
				{
